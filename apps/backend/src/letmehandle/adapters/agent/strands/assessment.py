...
Strict where strictness is the difference between a judgement and a guess. Every field is required,
so an answer cut off halfway is refused rather than completed with defaults nobody chose. A flag is
...
type _Intent = Annotated[CallIntent, _one_of([intent.value for intent in CallIntent])]
...
class CallAssessment(BaseModel):
    ...
    requested_capability: _Capability | None
    caller_summary: StrictStr | None
