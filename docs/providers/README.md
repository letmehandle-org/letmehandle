...
`bridging(transport)` — so a caller that has not checked cannot name the method, and a transport
whose declaration and implementation disagree fails at the narrowing rather than in the middle of
somebody's call.
